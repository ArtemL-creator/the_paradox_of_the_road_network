...
class Link:
    ...
    def drive(self):
        if self.car_queue.length() > 0:
            ...
            if car.progress >= self.path_length:
                self.car_queue.dequeue()
                self.destination_node.accept(car)
                self.update_speed()

                # Record travel time for the road
                self.total_travel_time += self.travel_time
                self.car_count += 1

    def get_avg_travel_time(self):
        return self.total_travel_time / self.car_count if self.car_count > 0 else 0
...
class Node:
    ...
    def dispatch(self):
        if self.car:
            self.next_link = self.car.route.itinerary[self.id]
            if len(self.next_link.car_queue) == 0 or self.next_link.car_queue[-1].progress >= car_length:
                self.car.progress = 0
                self.next_link.car_queue.append(self.car)
                self.next_link.update_speed()
                self.car = None
...
def choose_route():
    ...
    if not bridge_blocked:
        if sn_link.open_to_traffic:
            available_routes.extend([AB, ab])
    available_routes.extend([Ab, aB])
...
def launch_car():
    ...
    if global_clock >= next_departure and model_state == "running" and parking_lot.length() > 0:
        ...
        next_departure = global_clock + launch_rate
...
def simulate():
    ...
    # Calculate and print average travel times for each link
    print("\nAverage travel times per road:")
    for link in [a_link, b_link, sn_link, ns_link]:
        if link.car_count > 0:
            avg_time = link.get_avg_travel_time()
            print(f"Road {link.id}: {avg_time:.3f}s")
