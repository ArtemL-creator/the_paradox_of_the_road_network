...
class Dashboard:
    ...
    def record_arrival(self, car):
        ...
        print(f"Car {car.serial_number} arrived via {route_code}. Time: {elapsed}")
...
class Route:
    ...
    def calc_travel_time_historical(self):
        if self.dashboard.counts[self.label] == 0:
            self.travel_time = self.route_length / speed_limit
        else:
            # Берем среднее время из статистики и пересчитываем в "текущее" время
            avg_actual_time = self.dashboard.times[self.label] / self.dashboard.counts[self.label]
            self.travel_time = self.dashboard.times[self.label] / self.dashboard.counts[self.label]
        print(f"Route {self.label} historical time: {self.travel_time}")
...
class Link:
    def __init__(self, id_str, origin, destination, path_length):
        ...
        self.speed = speed_limit / (1 + congestion_coef * self.car_queue.length())
    ...
    def update_speed(self):
        # Учитываем коэффициент затора и длину очереди
        self.speed = speed_limit / (1 + congestion_coef * self.car_queue.length())
        self.travel_time = self.path_length / self.speed
    ...
    def drive(self):
        ...
        first_car.progress = round(first_car.progress)  # Округление
        ...
        for i in range(1, self.car_queue.length()):
            ...
            follower.progress = round(follower.progress)  # Округление
...
def choose_route():
    ...
    for route in available_routes:
        ...
        print(f"Route {route.label}: travel_time = {route.travel_time:.2f}")
...
def save_stats():
    ...
    # for route in routes:
    #     count = dashboard["counts"][route]
    #     avg_time = (
    #         dashboard["times"][route] / count if count > 0 else "--"
    #     )
    #     print(f"{route}: Count = {count}, Avg Time = {avg_time}")
    # total_count = dashboard["counts"]["total"]
    # total_time = (
    #     dashboard["times"]["total"] / total_count if total_count > 0 else "--"
    # )
    # print(f"Total: Count = {total_count}, Avg Time = {total_time}")
...
def steps():
    for _ in range(car_queue_size):
        ...
        step_counter += 1
        for route in [Ab, aB, AB, ab]:
            route.calc_travel_time()  # Обновляем время на каждом шаге
        check_simulation_conditions()
        print(
            f"Queues: A_link={A_link.car_queue.length()}, a_link={a_link.car_queue.length()}, B_link={B_link.car_queue.length()}, b_link={b_link.car_queue.length()}")
...
def check_simulation_conditions():
    ...
    # Условие 1: Симуляция останавливается, когда машины паркуются
    # if model_state == "stopping" and parking_lot.car_queue.length() == car_queue_size:
    #     animation_timer.cancel()  # Останавливаем таймер или цикл
    #     model_state = "stopped"
    #     save_stats()
    #     print("Simulation stopped: all cars returned to parking lot.")
    #
    # # Условие 2: Симуляция переходит в состояние остановки, если достигнут лимит машин
    # if model_state == "running" and dashboard.departure_count >= max_cars:
    #     model_state = "stopping"
    #     print("Simulation is stopping: maximum number of cars reached.")
...
if __name__ == '__main__':
    ...
    Ab = Route(label="Ab", dashboard=dashboard)
    Ab.directions = {
        "orig": A_link,
        "south": None,
        "north": b_link,
    ...
    print(f"AB route length: {AB.route_length}")
    print(f"Ab route length: {Ab.route_length}")
    print(f"aB route length: {aB.route_length}")
    print(f"ab route length: {ab.route_length}")
    ...
    init()
    step_counter = 0
    # animation_timer = launch_timer(launch_rate)
    steps()

    routes = ["Ab", "aB", "AB", "ab"]
    counts = [dashboard.counts[r] for r in routes]
    plt.bar(routes, counts)
    plt.title("Распределение машин по маршрутам")
    plt.show()
