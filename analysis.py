...
if not analysis_df.empty:
    # --- Первый график: Интенсивность vs. Разница времени ---
    plt.figure(figsize=(12, 7)) # Создаем новую фигуру для первого графика
    ...
    plt.ylabel('Разница в нормализованном среднем времени (Время_Открыт - Время_Закрыт)')
    plt.grid(True, which='both', linestyle=':', linewidth=0.5)
    plt.legend(title='Обозначения:', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()

    # --- Второй график: Коэффициент Сопротивления vs. Разница времени ---
    plt.figure(figsize=(12, 7)) # Создаем новую фигуру для второго графика

    sns.scatterplot(
        data=analysis_df,
        x='Коэф. Сопрот-я_rounded', # Меняем ось X на 'Коэф. Сопрот-я_rounded'
        y='Разница_Время_(Открыт - Закрыт)',
        hue='Метод Выбора',
        style='Режим Скорости',
        s=100,
    ...
    plt.show() # Эта команда отобразит оба графика в отдельных окнах
else:
    print("\nНет данных в analysis_df для построения графика.")
